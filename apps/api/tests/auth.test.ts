...
describe("Proofly API — Auth", () => {
  ...
  describe("POST /api/auth/login", () => {
    it("Should return 200 and auth tokens on valid credentials", async () => {
      ...
      vi.mocked(sessionQueries.enforceSessionLimit).mockResolvedValue(
        undefined,
      );
  ...
  describe("POST /api/auth/refresh", () => {
    ...
    it("Should return 401 if token is expired in db", async () => {
      ...
      expect(res.status).toBe(401);
      expect(sessionQueries.deleteSession).toHaveBeenCalledWith(
  ...
  describe("POST /api/auth/logout", () => {
    it("Should clear cookie and return 200 JSON Success", async () => {
      vi.mocked(sessionQueries.deleteSession).mockResolvedValue(undefined);
      ...
      expect(json.message).toBe("Logout Success");

      expect(res.headers.get("Set-Cookie")).toContain(
        "proofly_refresh_token=;",
      ); // deleted
